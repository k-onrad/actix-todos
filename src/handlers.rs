...
pub async fn status() -> impl Responder {
    web::HttpResponse::Ok()
        .json(models::Status { status: "UP".to_string() })
}
...
pub async fn create_todo_list(db_pool: web::Data<Pool>, json: web::Json<models::TodoListCreate>) -> impl Responder {
    let client: Client = db_pool.get().await.expect("Error connecting to the database");
...
pub async fn create_todo(db_pool: web::Data<Pool>, path: web::Path<(i32,)>, json: web::Json<models::TodoCreate>) -> impl Responder {
    let client: Client = db_pool.get().await.expect("Error connecting to the database");
